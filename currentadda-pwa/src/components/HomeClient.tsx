...
export default function HomeClient({ initialQuizzes = [] }: { initialQuizzes?: any[] }) {
    ...
    const fetchQuizzes = async (pageNumber: number, reset: boolean = false) => {
        if (reset) {
            // Don't clear immediately if we are just refining a search, avoids flash
            setLoading(true);
    ...
    // Prevent hydration mismatch by handling auth loading, but show content if we have data (SSR)
    // We only block if we really need to know auth state for the first paint, 
    // but for SEO/LCP of the public list, we might want to render that part anyway.
    // However, the header depends on 'user'.
    // Using HeroSkeleton is fine for the auth transition.
