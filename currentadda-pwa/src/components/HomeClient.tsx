...
export default function HomeClient({ initialQuizzes = [] }: { initialQuizzes?: any[] }) {
    ...
    const fetchQuizzes = async (pageNumber: number, reset: boolean = false) => {
        ...
        try {
            ...
            if (data) {
                ...
                else setHasMore(data.length === ITEMS_PER_PAGE);
            }
        } catch (err) {
    ...
    const fetchUserStats = async () => {
        try {
            ...
            if (data) {
                ...
                setUserStats({ totalQuizzes, totalScore, accuracy, totalQuestions });
            }
        } catch (err) {
    ...
    return (
        <LazyMotion features={domAnimation}>
            {(authLoading && !initialQuizzes.length) ? (
                <HeroSkeleton />
            ) : (
                            ...
                            >
                                ...
                                {userStats && (
                                    <div className="grid grid-cols-3 gap-3">
                                        <StatCard icon={<Trophy className="w-4 h-4" />} value={userStats.totalQuizzes} label="Quizzes" gradient="from-amber-400 to-orange-500" />
                                        <StatCard icon={<Target className="w-4 h-4" />} value={`${userStats.accuracy}%`} label="Accuracy" gradient="from-indigo-400 to-purple-500" />
                                        <StatCard icon={<Award className="w-4 h-4" />} value={userStats.totalScore} label="Points" gradient="from-emerald-400 to-teal-500" />
                                    </div>
                                )}
                            </m.div>
                        </section>
                        ...
                        {initialLoading ? (
                            ...
                            >
                                ...
                                <div className="flex gap-2 overflow-x-auto pb-1 no-scrollbar">
                                    <button onClick={() => setSelectedMonth(null)} className={`px-4 py-2 rounded-xl text-[9px] font-black uppercase tracking-widest whitespace-nowrap transition-all flex-shrink-0 ${selectedMonth === null ? 'bg-slate-900 text-white shadow-lg' : 'bg-slate-50 text-slate-400 hover:bg-slate-100'}`}>All Time</button>
                                    {recentMonths.map((date) => {
                                        ...
                                        return (
                                            <button
                                                ...
                                                className={`px-4 py-2 rounded-xl text-[9px] font-black uppercase tracking-widest whitespace-nowrap transition-all flex-shrink-0 ${isActive ? 'bg-gradient-to-r from-indigo-600 to-purple-600 text-white shadow-lg' : 'bg-slate-50 text-slate-400 hover:bg-slate-100'}`}
                                            >
                                                {format(date, 'MMM yy')}
                                            </button>
                                        );
                                ...
                                <>
                                    ...
                                    <AnimatePresence mode="popLayout">
                                        {quizzes.slice(1).map((quiz, index) => {
                                            ...
                                            const displayTitle = isIndiaBix && !quiz.title.includes('IndiaBix') ? `IndiaBix - ${quiz.title}` : quiz.title;
                                                        ...
                                                        <div className="flex justify-between items-center">
                                                            <div className="flex-1">
                                                                <div className="flex items-center gap-2 mb-2">
                                                                    ...
                                                                    {isIndiaBix && (
                                                                        <span className="text-[7px] font-black bg-orange-50 text-orange-600 px-2 py-0.5 rounded-lg uppercase tracking-tighter">IndiaBix</span>
                                                                    )}
                                                                </div>
